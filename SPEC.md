...
| Tests | `vitest`; für Komponenten-Tests mit echter Nutzerinteraktion (z. B. Auf-/Zuklappen-Verhalten) zusätzlich `@testing-library/react` + `jsdom` (Test-Environment nur für die betroffene Datei via `// @vitest-environment jsdom`). Ausnahmen zu "keine zusätzlichen Libraries", da im Projekt kein Test-Runner vorhanden war; alle drei nur als `devDependency`, kein Einfluss auf den Produktions-Build |
...
| 2026-08-14 | Etappe 10.1 (Refactor): Die Einstellungs-Zeile für die kritische Phase (§2.2) ist jetzt zusammenklappbar – zusammengeklappt (Default) eine einzige Zeile im Stil der Sektions-Beschriftungen mit der aktuellen Einstellung als Klartext (z. B. "Kritische Phase: 7 Tage vorher – Vollmond") und Chevron-Icon; aufgeklappt erscheinen darunter sanft ein-/ausblendend (`prefers-reduced-motion`-bewusst) die bestehenden Selects. Der Auf-/Zu-Zustand ist flüchtiger UI-State, die Phasen-Einstellung selbst bleibt persistent. Grund: Die selten geänderte Einstellung brach auf 390px in eine zweite Zeile um. §4: `@testing-library/react` + `jsdom` als weitere, auf Komponenten-Interaktionstests beschränkte Ausnahme zu "keine zusätzlichen Libraries" ergänzt. |
